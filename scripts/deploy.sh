...
# 5. Bring up services except Nginx (nginx requires SSL cert folder initially)
echo -e "${GREEN}==> Spinning up backend, database, and cache worker services...${NC}"
$DC -f docker-compose.prod.yml up -d db redis backend celery_worker celery_beat frontend

# 6. Database Migrations & Assets Compilation
...
if [ "$DOMAIN_NAME" != "localhost" ] && [ -n "$DOMAIN_NAME" ]; then
    ...
    mkdir -p nginx/ssl/live/studycommander.ai
    openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
        -keyout nginx/ssl/live/studycommander.ai/privkey.pem \
        -out nginx/ssl/live/studycommander.ai/fullchain.pem \
